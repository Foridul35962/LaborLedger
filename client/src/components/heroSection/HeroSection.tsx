...
import {
  ...
  PrevButton,
  usePrevNextButtons,
} from "../ui/emblaCarouselButton/EmblaCarouselArrow";
        ...
        <div className="flex touch-pan-y touch-pinch-zoom">
          {heroData.map((item) => (
            <div key={item.id} className="flex-[0_0_100%] relative h-150">
              ...
              />
              <div className="absolute top-1/2 left-0 flex flex-col px-10 sm:pl-15 sm:w-1/2 text-white transform -translate-y-1/2">
                <span className="text-lg font-bold text-yellow-500">
                  {item.subTitle}
                </span>
                <h2 className="text-xl sm:text-3xl font-bold">{item.title}</h2>
                <p className="text-sm pt-4 sm:pt-0 sm:text-lg">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
